...
export default function WasteComposition({ composition, zoneType }) {
  ...
  const pieData = Object.entries(composition).map(([key, val]) => ({
    name: LABELS[key], value: val.percentage, weight: val.weight, color: COLORS[key],
  }));

  const zoneLabel = zoneType?.replace('_', ' ') || 'Unknown';
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <AnimatedCard delay={0.1} hover={false}>
          <h3 className="mb-2 text-lg font-semibold" style={{ color: 'var(--text-primary)' }}>Composition Breakdown</h3>
          <p className="mb-4 text-sm capitalize" style={{ color: 'var(--text-secondary)' }}>Zone: {zoneLabel}</p>
        ...
        <AnimatedCard delay={0.2} hover={false}>
          ...
          <div className="space-y-6">
            {Object.entries(composition).map(([key, val], i) => {
